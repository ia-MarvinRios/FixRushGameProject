...
namespace FixRushGame
{
    ...
    {
        [Header("Car Settings")]
        [SerializeField] FixRushGame.GatoRoot[] _gatoRoots;
        [SerializeField] float _gatoCheckerRadius = 0.3f;
        [SerializeField] FixRushGame.WheelRoot[] _wheelRoots;
        [Space(10)]
        ...
        internal FixRushGame.GatoRoot[] GatoRoots { get { return _gatoRoots; } }
        internal FixRushGame.WheelRoot[] WheelRoots { get { return _wheelRoots; } }
        ...
        IEnumerator FixCarCoroutine()
        {
            ...
            yield return StartCoroutine(FixRushGame.VManager.Instance.MoveToEndPoint(Agent, this));
        }
        ...
        public void PlaceGato(AuxPlayer player, int gatoRootIndex)
        {
            ...
            // Cache
            ref FixRushGame.GatoRoot root = ref _gatoRoots[gatoRootIndex];
        ...
        private void OnDrawGizmosSelected()
        {
            if (_showGatoRoots)
            {
                foreach (FixRushGame.GatoRoot gatoRoot in _gatoRoots)
                {
            ...
            {
                Gizmos.color = Color.white;
                foreach (FixRushGame.WheelRoot wheel in _wheelRoots)
                {
